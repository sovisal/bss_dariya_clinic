<tr>
	...
	<td>
		<x-bss-form.textarea name="liver" class="my-simple-editor">{{ old('liver', !empty($row) && $row->liver ? $row->liver : '<p>Hepatomegali diameter~ 145.9mm ,echo structure presence 2 nodular hyper echo gene</p><p>diameter ~53,7 mm x 59,2mm (2) diameter~29,7 x 29,9 in right lobe.</p><p>No dilatation of the intra hepatic bile duct is seen, the common bile duct is normal in diameter</p><p>The diameter of the aorta is normal and no aneurysms are seen.</p>') }}</x-bss-form.textarea>
	</td>
</tr>
<tr>
...
<tr>
	...
	<td>
		<x-bss-form.input name="impression" :value="old('impression', !empty($row) && $row->impression ? $row->impression : 'CHC')"/>
	</td>
</tr>