<tr>
	...
	<td>
		<x-bss-form.textarea name="liver" class="my-simple-editor">{{ old('liver', !empty($row) && $row->liver ? $row->liver : '<p>normal of size, homogeneous, echo structure no focal lesion is seen. </p><p>No dilatation of the intra hepatic bile duct is seen, the common bile duct is normal in diameter</p><p>The diameter of the aorta is normal and no aneurysms are seen.</p>') }}</x-bss-form.textarea>
	</td>
</tr>
<tr>
...
<tr>
	...
	<td>
		<x-bss-form.input name="kidneys" :value="old('kidneys', !empty($row) && $row->kidneys ? $row->kidneys : 'Presence an echo gene a cyst diameter ~ 26,9mm x 2,0mm . No kidney stones are seen. No blockage to the system draining the kidneys is present.')"/>
	</td>
</tr>
<tr>
...
<tr>
	...
	<td>
		<x-bss-form.input name="impression" :value="old('impression', !empty($row) && $row->impression ? $row->impression : 'RIGHT RENAL CYST')"/>
	</td>
</tr>