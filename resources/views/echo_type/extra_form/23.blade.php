...
<tr>
	...
	<td>
		<x-bss-form.input name="star1" :value="old('star1', !empty($row) && $row->star1 ? $row->star1 : 'Left kidney CALCIFICATION')"/>
	</td>
</tr>
<tr>
	<td class="text-right">*</td>
	<td>
		<x-bss-form.input name="star2" :value="old('star2', !empty($row) && $row->star2 ? $row->star2 : 'Right kidney MULTIPLE STONE')"/>
	</td>
</tr>
<tr>
	<td class="text-right">- Bladder</td>
	<td>
		<x-bss-form.input name="bladder" :value="old('bladder', !empty($row) && $row->bladder ? $row->bladder : 'full of urine')"/>
	</td>
</tr>
<tr>
...
<tr>
	<td class="text-right">*</td>
	<td>
		<x-bss-form.input name="star3" :value="old('star3', !empty($row) && $row->star3 ? $row->star3 : 'No abnormal growths are seen. No fluid is found in the abdomen.')"/>
	</td>
</tr>
<tr>
	<td class="text-right">IMPRESSION</td>
	<td>
		<x-bss-form.textarea name="impression" class="my-simple-editor">{{ old('impression', !empty($row) && $row->impression ? $row->impression : '<p>-MILD HYDRONEPHROSIS</p><p>-RIGHT  KIDDNEY STONE</p>') }}</x-bss-form.textarea>
	</td>
</tr>